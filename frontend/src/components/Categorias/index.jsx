...
import Eletrodomestico from '../../assets/img/eletrodomestico.png';
...
const categories = [
    ...
    {name: 'Eletrodomestico', img: Eletrodomestico},
...
export default function Categories({selectedBtn, onSelect}) {
    return (
        <section className="categories">
            {categories.map((cat) => (
                <button key={cat.name} onClick={() => onSelect(cat.name)} className={`btn ${selectedBtn === cat.name ? 'selectedBTn' : ''}`}>
                    <img src={cat.img} alt={cat.name}/><span>{cat.name}</span>
                </button>
            ))}
        </section>
    )
